#from httplib import HTTPConnection, HTTPSConnection #python2.x
import http.client
...
class MsfRpcClient(object):
    ...
    def __init__(self, password, **kwargs):
        ...
        self.server = kwargs.get('server', '10.108.36.138')
        self.ssl = kwargs.get('ssl', False)
        self.sessionid = kwargs.get('token')
        if self.ssl:
            self.client = http.client.HTTPSConnection(self.server, self.port)
        else:
            self.client = http.client.HTTPConnection(self.server, self.port)
        self.login(kwargs.get('username', 'bupt'), password)
